...
[data set](https://data.vancouver.ca/datacatalogue/crime-data.htm)
...
```
![Vacouver](https://en.wikipedia.org/wiki/Vancouver#/media/File:Concord_Pacific_Master_Plan_Area.jpg)
 ...
 *What type of crime appears most ?
 ...
 Are neighbourhoods targeted for certian crimes ? 
   -> Is type distributed equally among them
...
ggplot(count_of_crimes, aes(x = reorder(TYPE,total_of_crime), y = sqrt(total_of_crime) )) + 
  ...
  labs( y = "Square Root of Total Crime", x = "Type of Crime Committed", title = "Crime Distribution", subtitle = "Vancouver 2003-2019") +
  geom_text(aes(label = round(sqrt(total_of_crime)), y = sqrt(total_of_crime) + 15)) + #adds num to bar
  theme_simple()

  
  #labels for x,y, and title || Also see about scaling the size of graphs
...
# Overall crime at hours over the years
crime_all_years %>%
ggplot( aes(x = HOUR)) +
  geom_bar(fill = "steelblue") +
  labs(title = "Count of Crimes at Hour", subtitle = "By Years") +
  facet_wrap(~YEAR)

```
Pretty steady trend throughout the years, the hour of when a crime is committed seems constant. The chance of a crime happening is more likely in the evening of the day. Later we should look into if crime has been steady over the years or if there's any significant increase/decrease.
