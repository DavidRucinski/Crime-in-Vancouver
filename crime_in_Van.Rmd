...
# Can I make this interactive? drag bar for year/month?
```
...
ts_test <- crime_tidy %>%
 ...
 ts_test %>% group_by(month=floor_date(DATE, "month"),NEIGHBOURHOOD) %>%
   summarize(amount=sum(n)) %>%
   #select(amount > 200) %>% #amount doesnt exist.. why
