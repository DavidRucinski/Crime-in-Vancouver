...
list_over <- list_over[,1]
list_over <- as.list(list_over)
...
under_200 <- crime_tidy %>%
  filter(NEIGHBOURHOOD != "",NEIGHBOURHOOD != "Central Business District", NEIGHBOURHOOD != list_over) %>%
  group_by(NEIGHBOURHOOD) %>%
  count(DATE)


under_200 <- as.data.frame(under_200) 
