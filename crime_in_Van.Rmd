...
## **Summary** [CHANGE]
**Theft of Vehicle** is the most common crime to happen in Vancouver, given this data. In general **17:00** seems to be the peak time for a crime to be committed, although certain crimes favour different hours. The **Central Business District** seems to have the most crime committed over the years, [checking relative increase over the years].
...
*-Change TYPE, NEIGHBOURHOOD as factor*
...
```
Interesting, looks normally distributed with a left tail.
...
```
With all the coordinates from crimes it almost makes a full map of Vancouver. There is certain sections on that map that have a clear form. The bottom right for example,**Killarney**, has 2 void spots. They happen to be a gold course and a wooded park that has an off-leash area.
...
```{r time series of neighbourhood crime}
...
list_over <- list("", "Central Business District", "Fairview" , "Grandview-Woodland" , "Hastings-Sunrise","Kensington-Cedar Cottage","Kitsilano", "Mount Pleasant"  ,"Renfrew-Collingwood " ,"West End" )
  
  


  crime_tidy %>%
  filter(NEIGHBOURHOOD != list_over) %>% # how to pick var with list wtf
  group_by(NEIGHBOURHOOD) %>%
  count(DATE) %>% 
  group_by(NEIGHBOURHOOD, month=floor_date(DATE, "month")) %>%
  summarize(amount=sum(n)) %>%
...
# make a list then can split the two and make 3 graphs: central will get its own
...
#ggplot( data = ts_test, aes( DATE, n )) + 
#  geom_line()
...
```{r homicide over time}

offset[TYPE == "Homicide" , .N, by = .(TYPE, YEAR)] %>%
  ggplot( aes(x = YEAR, y = N)) +
  geom_line() +
  coord_cartesian(xlim = c(2003,2019) )
